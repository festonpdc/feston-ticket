...
function mapStripeStatus(status: Stripe.PaymentIntent.Status, method: PaymentMethod): PaymentStatus { if (status === 'succeeded') return 'paid'; if (status === 'processing') return method === 'oxxo' ? 'awaiting_cash' : 'processing'; if (status === 'canceled') return 'cancelled'; if (status === 'requires_payment_method') return 'pending'; return 'pending'; }
export class StripePaymentProvider implements PaymentProvider {
  private readonly stripe: Stripe; private readonly webhookSecret: string | undefined;
  constructor(secretKey = process.env.STRIPE_SECRET_KEY, webhookSecret = process.env.STRIPE_WEBHOOK_SECRET) { if (!secretKey) throw new Error('Stripe server environment is not configured'); this.stripe = new Stripe(secretKey); this.webhookSecret = webhookSecret; }
  async createPaymentIntent(input: PaymentIntentInput): Promise<PaymentIntentResult> {
    ...
    const intent = await this.stripe.paymentIntents.create({ amount: input.amount, currency: input.currency.toLowerCase(), payment_method_types, metadata: { order_id: input.orderId, ...(input.organizationId ? { organization_id: input.organizationId } : {}), ...(input.eventId ? { event_id: input.eventId } : {}) } }, { idempotencyKey: input.idempotencyKey });
