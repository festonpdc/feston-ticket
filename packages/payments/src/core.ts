...
export async function processVerifiedStripeEvent(repository: PaymentRepository, event: { id: string; type: string; created: number; paymentIntentId: string; amount: number; currency: string; method: PaymentMethod; voucherExpiresAt?: string; voucherUrl?: string }): Promise<'applied' | 'duplicate' | 'rejected' | 'reconciliation_required'> {
  if (!['payment_intent.succeeded', 'payment_intent.payment_failed', 'payment_intent.processing', 'payment_intent.requires_action', 'payment_intent.canceled'].includes(event.type)) return 'rejected';
  if (!Number.isSafeInteger(event.created) || event.created <= 0) return 'rejected';
  if ((event.type === 'payment_intent.requires_action' || event.type === 'payment_intent.canceled') && event.method !== 'oxxo') return 'rejected';
  if (event.type === 'payment_intent.requires_action' && (!event.voucherExpiresAt || !event.voucherUrl)) return 'rejected';
  return repository.applyVerifiedEvent({ eventId: event.id, eventType: event.type, providerPaymentId: event.paymentIntentId, providerEventCreatedAt: new Date(event.created * 1000).toISOString(), amount: event.amount, currency: event.currency.toUpperCase(), method: event.method, ...(event.voucherExpiresAt ? { voucherExpiresAt: event.voucherExpiresAt } : {}), ...(event.voucherUrl ? { voucherUrl: event.voucherUrl } : {}) });
}
