...
describe('Stripe webhook route', () => {
  ...
  it('passes a verified card success through the public financial bridge', async () => {
    verifyWebhookMock.mockReturnValue({ id: 'evt_1', type: 'payment_intent.succeeded', created: 1790366087, data: { object: { id: 'pi_1', amount: 54000, currency: 'mxn', payment_method_types: ['card'] } } });
    ...
    expect(rpc).toHaveBeenCalledWith('apply_stripe_payment_event', expect.objectContaining({
      ...
      p_currency: 'MXN', p_method: 'card', p_provider_event_created_at: '2026-09-25T19:54:47.000Z',
    }));
  });

  it('never interprets a failed payment as paid', async () => {
    verifyWebhookMock.mockReturnValue({ id: 'evt_2', type: 'payment_intent.payment_failed', created: 1790366087, data: { object: { id: 'pi_2', amount: 54000, currency: 'mxn', payment_method_types: ['card'] } } });
