...
export function emailTicketAccessUrl(organizationId:string,orderId:string,expiresAt?:number){const base=process.env.APP_URL;if(!base)throw new Error('APP_URL is required');return new URL(`/entradas/${encodeURIComponent(issueEmailTicketAccess(organizationId,orderId,expiresAt))}`,base).toString()}
