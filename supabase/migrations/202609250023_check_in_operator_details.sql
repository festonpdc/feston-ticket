...
begin
  ...
  select name into ticket_type_name from public.ticket_types where id=ticket_row.ticket_type_id and organization_id=p_organization_id;

  select checked_in_at into existing_at from public.check_ins where ticket_id=ticket_row.id;
  if found then
    return jsonb_build_object('result','already_checked_in','checked_in_at',existing_at,'ticket_type_name',ticket_type_name,'public_code',ticket_row.public_code);
  end if;
  ...
  return jsonb_build_object('result','accepted','checked_in_at',accepted_at,'ticket_type_name',ticket_type_name,'public_code',ticket_row.public_code);
end;
$$;
